...
def main():
    ...
    if args.random_split_train_val:
        ...
        # val_indices = indices[int(num_train_val * 0.1):]
    ...
    if config.train.loss == 'NCLoss':
        ...
        criterion = NCLoss(sup_criterion=config.train.sup_criterion, lambda1=config.train.lambda1, lambda2=config.train.lambda2, lambda_CE=config.train.lambda_CE, num_classes=config.model.out_dim, feat_dim=config.model.hidden_dims[-1], device=args.device, nc1=config.train.nc1, nc2=config.train.nc2, occurance_list=occurance_list, fixed_means=config.train.fixed_means, weight_factor=config.train.weight_factor)
    ...
    multilabel_testset = MultiLabelDataset(config.data.test_data_file, config.data.label_file, config.data.label_name, config.data.label_level, logger=logger)
    ...
    os.system(f'python scripts/infer_NC_lookup.py configs/infer.yml --device {args.device} --model_dir {log_dir} --lookup_data {config.data.train_data_file if not hasattr(config.data, "original_train_data_file") else config.data.original_train_data_file} --test_data {config.data.test_data_file} --label_file {config.data.label_file} --topk 1 --ont {config.data.label_name}')
