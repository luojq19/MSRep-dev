...
def infer_lookup_query(lookup_emb, lookup_labels, query_emb, query_labels, query_pids, topk, logger):
    ...
    return pid2pred, pid2gt, scores

def save_predictions(pid2pred, pid2gt, scores, save_path):
    pids = list(pid2pred.keys())
    predictions = [';'.join(pid2pred[pid]) for pid in pids]
    ground_truths = [';'.join(pid2gt[pid]) for pid in pids]
    df = pd.DataFrame({'Entry': pids, 'Predictions': predictions, 'Ground Truth': ground_truths, 'Scores': scores})
...
def get_args():
    ...
    parser.add_argument('--output', type=str, default='predictions_lookup_scores.csv')
...
def main():
    ...
    pid2pred, pid2gt, scores = infer_lookup_query(lookup_emb, lookup_labels, test_emb, test_labels, test_pids, config.topk, logger)
    evaluate(pid2pred, pid2gt, config.label_file, logger)
    
    save_predictions(pid2pred, pid2gt, scores, os.path.join(config.model_dir, args.output))
