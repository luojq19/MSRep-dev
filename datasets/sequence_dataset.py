...
class MLabLossDataset(Dataset):
    def __init__(self, data_file, single_label_list_file, multi_label_list_file, label_name='ec', label_level=4, logger=None) -> None:
        super().__init__()
        self.logger = logger if logger is not None else get_logger('MLabLossDataset')
        self.raw_data = torch.load(data_file)
        self.pids = list(self.raw_data.keys())
        self.embeddings = [self.raw_data[pid]['embedding'] for pid in self.pids]
        self.raw_labels = [self.raw_data[pid][label_name] for pid in self.pids]
        with open(single_label_list_file, 'r') as f:
            self.single_label_list = json.load(f)
        with open(multi_label_list_file, 'r') as f:
            self.multi_label_list = json.load(f)
        self.label_list = self.single_label_list + self.multi_label_list
        self.label2idx = {l: i for i, l in enumerate(self.label_list)}
        self.single_label2idx = {l: i for i, l in enumerate(self.single_label_list)}
        self.multi_label_list_split = [l.split(';') for l in self.multi_label_list]
        self.multi_class_idx_list = [[self.single_label2idx[l] for l in labels] for labels in self.multi_label_list_split]
        self.labels = []
        for i, labels in enumerate(self.raw_labels):
            merged_label = ';'.join(labels)
            self.labels.append(self.label2idx[merged_label])
        self.logger.info(f'Loaded {len(self.raw_data)} sequences with {label_name} labels')
        self.logger.info(f'Number of single labels: {len(self.single_label_list)}')
        self.logger.info(f'Number of multi labels: {len(self.multi_label_list)}')
        self.logger.info(f'Number of all labels: {len(self.label_list)}')
        self.num_labels = len(self.label_list)
        self.num_single_labels = len(self.single_label_list)
        self.num_multi_labels = len(self.multi_label_list)
...
if __name__ == '__main__':
    dataset = MLabLossDataset(data_file='../data/ec_new/swissprot_ec_complete_by2022-05-25_val.pt', single_label_list_file='../data/ec_new/ec_list_by2022-05-25.json', multi_label_list_file='../data/ec_new/ec_list_multilabel_by2022-05-25.json', label_name='ec', label_level=4)
